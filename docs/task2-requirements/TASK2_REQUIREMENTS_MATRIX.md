...
- **Baseline:** 95/95 vitest tests pass; production build and `tsc --noEmit` both clean.
...
| B4 | Validate Stripe setup process | SOW | **SCOPED — build, not validation** | Full assessment in [STRIPE_SCOPE_TASK2B.md](STRIPE_SCOPE_TASK2B.md). Confirmed absent: no `stripe` dependency, no Stripe code (only two *"no Stripe"* comments), **no `STRIPE_SECRET_KEY` in project secrets**, no checkout/portal/webhook, no `subscriptions`/`customers`/`prices` tables. Groundwork that **does** exist: `invoices` (15 cols, RLS correct), 5 `profiles` subscription columns, all 37 profiles `starter`/`active`, an Invoices page already reading the table, and Resend verified for invoice email. **Scaffolding now built** (test mode): billing schema migration `20260925195000` (written, **not applied**), `_shared/stripe.ts`, and three edge functions — `create-checkout-session`, `create-billing-portal-session`, `stripe-webhook` (signature-verified, claim-first idempotent). Entitlement rules in `src/lib/subscriptionEntitlement.ts` with 9 tests. Runbook: [STRIPE_TEST_MODE_RUNBOOK.md](STRIPE_TEST_MODE_RUNBOOK.md). Functions return 503 `stripe_not_configured` until secrets exist. **Schema applied** (ledger 395). **Discovered: a Stripe integration is already deployed** — `stripe-webhook`, `stripe-checkout`, `send-invoice` (all v4, ACTIVE) exist on the project but were never committed; sources recovered to `supabase/functions/_recovered/`. **Four defects found and ported into them, all now deployed at v5:** conditional signature check (forged events could grant a paid plan), no idempotency guard (redelivery duplicated invoices), `priceId` taken from the request body (a caller could pick a cheaper price), and unescaped invoice fields in the receipt HTML. Verified live: a forged unsigned event is rejected. **Still blocked on a client action** — Stripe account, $49/mo price id, and the three secrets. |
| B5 | Validate User (subscribers) Invoicing system | SOW | **SCOPED** | `invoices` is fully shaped with correct RLS (`select_own` + `insert_admin`) but has **0 rows** and nothing writes `invoice_number`. Invoice rows must be created by the `invoice.paid` / `invoice.payment_failed` webhooks, keyed on `stripe_invoice_id` for idempotency, with the receipt emailed via the already-verified Resend domain. Depends on B4. The webhook scaffolding writes these rows on `invoice.paid` / `invoice.payment_failed`, and `next_invoice_number()` generates `IEP-000001`-style numbers. |
