  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      const transformedData = data.map((event, index) => ({
        id: `manage-event-${index}`, // Generate temp ID since table doesn't have id
        user_id: event.event_user_id,
        title: event.event_contact_name || 'Unnamed Event',
        ...
        theme_id: event.event_theme ? parseInt(event.event_theme) : undefined,
        type_id: event.event_type ? parseInt(event.event_type) : undefined,
        status: event.event_status,
        budget: event.event_budget_cost ? event.event_budget_cost[0] : undefined,
        venue: event.venue_name,
        created_at: event.created_at,
        updated_at: event.created_at // No updated_at field, using created_at
      }));
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .update({
          ...
          venue_location: eventData.location,
          venue_name: eventData.venue,
          event_theme: eventData.theme_id?.toString(),
          event_type: eventData.type_id?.toString(),
          event_status: eventData.status as 'pending' | 'in_progress' | 'completed' | 'cancelled',
          event_budget_cost: eventData.budget ? [eventData.budget] : null,
        })
        .eq('event_user_id', eventData.user_id);
