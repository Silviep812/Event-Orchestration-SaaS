...
const PERMISSION_LEVELS: Record<
  string,
  { label: string; shortLabel: string; icon: typeof Crown; color: string; description: string }
> = {
  admin: {
    label: "Owner/Admin (CRUD)",
    shortLabel: "CRUD",
    icon: Crown,
  ...
  coordinator: {
    label: "Create, Read and Update (CRU)",
    shortLabel: "CRU",
    icon: ClipboardList,
    color: "bg-primary/10 text-primary border-primary/20",
    description: "Can create, read and update events and tasks",
  },
  viewer: {
    label: "Read Only (R)",
    shortLabel: "R",
...
export function RoleManager({
  ...
  const [crInitiateDialog, setCrInitiateDialog] = useState<{ open: boolean; taskId: string | null; taskTitle: string }>(
  ...
  useEffect(() => {
    ...
    const crChannel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "change_requests" }, () => {
        if (isMounted) fetchChangeRequests();
      })
      .subscribe();
  ...
  const fetchUsers = async () => {
    try {
      ...
      const unassignedUsers = allUsers
        .filter((u: any) => !userRolesData?.find((role) => role.user_id === u.id))
        .map((u: any) => ({ ...u, role: "Member" }));
      setUsersWithoutRoles(unassignedUsers);

      const roleAssignments =
        userRolesData?.map((role: any) => ({
          id: role.id,
  ...
  const fetchTasks = async () => {
    try {
      let query = supabase
        .from("tasks")
        .select(
          "id, title, status, priority, due_date, assigned_to, assigned_coordinator_name, event_id, category, assignment_type, checklist",
        )
  ...
  const syncToCollaboratorConfig = async (userId: string, roles: string[], permissionLevel: PermissionLevel) => {
    ...
    try {
      ...
      if (existing) {
        ...
        await supabase.from("collaborator_configurations").insert({
    ...
    try {
      ...
      if (action === "approve") {
        ...
        if (applyError) {
          ...
          toast({
            title: "Warning",
            description: "Approved but failed to auto-apply changes.",
            variant: "destructive",
          });
        }
      }
      ...
      toast({
        ...
        description:
          action === "approve" ? "Change request approved and changes applied." : "Change request has been declined.",
      });
  ...
  const handleUpdateTaskStatus = async (taskId: string, newStatus: string) => {
    try {
      const { error } = await supabase
        .from("tasks")
        .update({
          status: newStatus as "not_started" | "in_progress" | "completed" | "on_hold" | "cancelled",
  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-0">
          ...
          <div className="hidden lg:block overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {/* OWNER/ADMIN ROWS */}
                {(permissionFilter === "all" || permissionFilter === "admin") &&
                  consolidated
                    .filter(
                      (cu) =>
                        cu.highestPermission === "admin" &&
                        (roleFilter === "all" || cu.roles.some((r) => r.role === roleFilter)),
                    )
                    .map((cu) => {
                ...
                {(permissionFilter === "all" || permissionFilter === "coordinator") &&
                  consolidated
                    .filter(
                      (cu) =>
                        cu.highestPermission === "coordinator" &&
                        (roleFilter === "all" || cu.roles.some((r) => r.role === roleFilter)),
                    )
                    .map((cu) => {
                      const userInfo = getUserInfo(cu.user_id);
                ...
                {/* VIEWER ROWS */}
                {(permissionFilter === "all" || permissionFilter === "viewer") &&
                  consolidated
                    .filter(
                      (cu) =>
                        cu.highestPermission === "viewer" &&
                        (roleFilter === "all" || cu.roles.some((r) => r.role === roleFilter)),
                    )
                    .map((cu) => {
                      const userInfo = getUserInfo(cu.user_id);
                ...
                {(() => {
                  const roleMatch = (cu: ConsolidatedUser) =>
                    roleFilter === "all" || cu.roles.some((r) => r.role === roleFilter);
                  const hasAdmin =
                    (permissionFilter === "all" || permissionFilter === "admin") &&
                    consolidated.some((cu) => cu.highestPermission === "admin" && roleMatch(cu));
                  const hasCoord =
                    (permissionFilter === "all" || permissionFilter === "coordinator") &&
                    consolidated.some((cu) => cu.highestPermission === "coordinator" && roleMatch(cu));
                  const hasViewer =
                    (permissionFilter === "all" || permissionFilter === "viewer") &&
                    consolidated.some((cu) => cu.highestPermission === "viewer" && roleMatch(cu));
                  if (!hasAdmin && !hasCoord && !hasViewer)
                    return (
                      <TableRow>
                        <TableCell colSpan={5} className="text-center py-12">
                          ...
                          <p className="text-xs text-muted-foreground">
                            Try adjusting the filters or invite team members to get started.
                          </p>
                        </TableCell>
                      </TableRow>
                    );
        ...
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => {
                if (rejectDialog.requestId) handleChangeRequestAction(rejectDialog.requestId, "reject");
              }}
            >
              Decline Request
            </Button>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Initiate Change Request</DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <div>
          ...
          <DialogFooter>
            ...
            <Button
              disabled={!crDescription.trim() || actionLoading === "cr-initiate"}
      ...
      <TableCell className="py-4">
        <div className="space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                size="sm"
                className="w-full justify-between font-bold text-[10px] h-8 border-primary/30 uppercase tracking-tight"
              >
                <span className="truncate">
                  {selectedLabels.length > 0 ? selectedLabels.join(", ") : "Select Role..."}
                </span>
            ...
            <PopoverContent className="w-[220px] p-2 bg-popover z-50 border-primary/20" align="start">
              <p className="text-[10px] font-bold uppercase text-muted-foreground px-2 mb-2 tracking-widest">
                Select Event Roles
              </p>
              {ROLES.map((role) => (
                <label
          ...
          {isDirty && (
            <Button
              size="sm"
              className="h-7 text-[10px] w-full bg-primary font-bold uppercase tracking-wider"
              onClick={handleSave}
            >
              Save Roles
            </Button>
          ...
          <div className="text-[11px] text-muted-foreground space-y-0.5 pt-1.5 border-t mt-2">
            <p>
              <span className="font-bold text-foreground uppercase text-[9px]">Event:</span>{" "}
              {selectedEvent?.title || "Global"}
            </p>
          </div>
        </div>
      </TableCell>
              ...
              return (
                <div key={cr.id} className="p-2.5 rounded-lg border bg-background/80 space-y-2">
                  <div className="flex items-center gap-1.5">
                    <Badge className="bg-accent text-accent-foreground border-accent text-[9px] rounded-full px-1.5">
                      CR
                    </Badge>
                    <Badge
                      variant="outline"
                      className={`text-[9px] rounded-full px-1.5 ${cr.priority === "high" || cr.priority === "critical" ? "border-destructive/50 text-destructive" : ""}`}
                    >
                      {cr.priority}
                    </Badge>
                  </div>
                    ...
                    <div className="bg-muted/30 rounded p-1.5 space-y-0.5">
                      {fieldEntries.slice(0, 3).map(([key, val]) => (
                        <p key={key} className="text-[10px]">
                          ...
                          <span className="line-through text-muted-foreground">"{val?.oldValue || ""}"</span> →{" "}
                          <span className="font-bold">"{val?.newValue || ""}"</span>
                        </p>
                      ))}
                    </div>
                  )}
  ...
  const getChecklistForTask = (task: TaskData): { items: string[]; category: string } | null => {
    ...
    for (const [catName, items] of Object.entries(grouped)) {
      if (
        ...
        category.toLowerCase().includes(catName.toLowerCase())
      ) {
    ...
    <TableRow className="align-top hover:bg-muted/10">
      ...
      <TableCell className="py-4">
        <div className="space-y-2">
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" size="sm" className="w-full justify-between font-normal text-xs h-8">
                <span className="truncate">
                  {selectedLabels.length > 0 ? `${selectedLabels.length} role(s)` : "Select..."}
                </span>
          ...
          {isDirty && (
            <Button size="sm" className="h-7 text-xs w-full" onClick={handleSave}>
              Save
            </Button>
          )}
        </div>
                ...
                <div key={t.id} className="space-y-2 p-2 rounded-md border bg-background/50">
                  ...
                  <div className="space-y-1 bg-muted/30 p-1.5 rounded-sm">
                    <p className="text-[10px] font-medium truncate">
                      {checklistInfo?.category || t.category || "Checklist"}
                    </p>
                    {total > 0 ? (
                      <>
                ...
                <div className="flex flex-wrap gap-1">
                  ...
                  <Select
                    onValueChange={(status: string) =>
                      onUpdateTaskStatus(
                        t.id,
                        status as "not_started" | "in_progress" | "completed" | "on_hold" | "cancelled",
                      )
                    }
                  >
